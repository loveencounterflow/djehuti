(function() {
  'use strict';
  var DATOM, Djehuti, Emittery, GUY, alert, debug, echo, get_base_types, get_xemitter_types, help, info, inspect, log, misfit, plain, praise, rpr, urge, warn, whisper;

  //###########################################################################################################
  GUY = require('guy');

  ({alert, debug, help, info, plain, praise, urge, warn, whisper} = GUY.trm.get_loggers('DATOM/DJEHUTI'));
  ...
  Djehuti = class Djehuti {
    ...
    _mark_as_primary(x) {
      return DATOM.wrap_datom('~DJEHUTI-preferred', {
        $key: '~wrapper',
        $value: x
      });
    }
    ...
    _filter_primary(x) {
      return DATOM.select(x, '~DJEHUTI-preferred');
    }
