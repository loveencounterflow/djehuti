(function() {
  'use strict';
  var AE, AE_Event, AE_Event_results, Async_events, Datom, SYMBOLIC, get_WeakMap, isa, isa_optional, resolved_promise, rpr, validate, validate_optional;
    ...
    on($key, receiver) {
      ...
      if (isa.event_listener(receiver)) {
        ...
        if (!isa.event_listener(listener0)) {
          /* TAINT `typeof` will give some strange results */
          throw new Error(`expected event_listener for object property ${rpr(listener_name)}, got a ${typeof listener0}`);
        }
