...
//.........................................................................................................
  ...
  Intertalk = class Intertalk {
    ...
    //---------------------------------------------------------------------------------------------------------
    off(listener) {
      var R, i, idx, key_symbol, ref, ref1, ref2, registered_key, registered_listeners_and_ctls, y;
      if (!isa.unary(arguments)) {
        /* TAINT add optional $key to unsubscribe only from specific note $key */
        throw new Error(`expected 1 arguments, got ${arguments.length}`);
      }
      validate.IT_listener(listener);
      R = 0;
      ...
      for (y of ref) {
        ...
        for (idx = i = ref2 = registered_listeners_and_ctls.length - 1; i >= 0; idx = i += -1) {
          if (registered_listeners_and_ctls[idx][0] !== listener) {
            continue;
          }
          R++;
    ...
    on_any(listener) {
      var ctl;
      validate.IT_listener(listener);
      ctl = this._get_ctl(this.symbols.any, listener);
      (this._listeners_from_key(this.symbols.any)).push([listener, ctl]);
      return null;
    }
    ...
    on_unhandled(listener) {
      var ctl;
      validate.IT_listener(listener);
      ctl = this._get_ctl(this.symbols.unhandled, listener);
      (this._listeners_from_key(this.symbols.unhandled)).push([listener, ctl]);
      return null;
    }
